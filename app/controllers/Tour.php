...
class Tour extends Controller {
   ...
   public function findTour($slug) {
      $dpt = htmlspecialchars(Request::input("departure", ""));
      $des = htmlspecialchars(Request::input("destination", ""));
      ...
      if (in_array($slug, ['tour-trong-nuoc', 'tour-nuoc-ngoai'])) {
         $this->data["destination"] = $this->LocationModel->where([
             'is_destination' => 1,
      ...
      if ($des) {
         $checkDestination = $this->LocationModel->find($des, "slug");
         if (!$checkDestination) {
            Util::loadError();
         }
      }
   ...
   public function searchProductAjax() {
      ...
      if (Request::has("destination", "get")) {
         ...
         $filters['destination'] = Util::generateSlug($destination);
      }
   ...
   private function loadCommonData() {
      ...
      $this->data['departures'] = $this->data["departure"]; 
   }
